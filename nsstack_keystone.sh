...
connection = mysql://keystone:$password@$hostname/keystone
...
keystone endpoint-create --service-id=$(keystone service-list | awk '/ network / {print $2}') --publicurl='http://'"$hostname"':9696' --adminurl='http://'"$hostname"':9696' --internalurl='http://'"$hostname"':9696'
...
# heat
keystone user-create --name=heat --pass=$password --email=$email
keystone user-role-add --user=heat --tenant=service --role=admin
keystone role-create --name heat_stack_user
keystone service-create --name=heat --type=orchestration --description="Orchestration"
keystone endpoint-create --service-id=$(keystone service-list | awk '/ orchestration / {print $2}') --publicurl='http://'"$hostname"':8004/v1/$(tenant_id)s' --internalurl='http://'"$hostname"':8004/v1/$(tenant_id)s' --adminurl='http://'"$hostname"':8004/v1/$(tenant_id)s'
keystone service-create --name=heat-cfn --type=cloudformation --description="Orchestration CloudFormation"
keystone endpoint-create --service-id=$(keystone service-list | awk '/ cloudformation / {print $2}') --publicurl='http://'"$hostname"':8000/v1' --internalurl='http://'"$hostname"':8000/v1' --adminurl='http://'"$hostname"':8000/v1'